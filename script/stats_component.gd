...
var parent_stats: PlayerStats = null
...
func _add_cooldown(action: String) -> void:
	...
	cooldown = clampf(cooldown+t, 0.0, parent_stats.max_cooldown) 
#endregion
...
func _handle_timers(delta: float) -> void:
	...
	if regen_is_accelerating:
		...
		if regen_accel_timer >= parent_stats.max_regen_time:
...
func _get_regen(rate: float, _delta: float) -> float:
	...
	var n: float = clampf(regen_accel_timer / parent_stats.max_regen_time, 0.0, 1.0)
